...
sub GetMake{
    ...
    if ($node_rights[2] =~ /／(.+)：強さ(\d+)／/) {
        ...
        $self->{Datas}{Data}->AddData(join(ConstData::SPLIT, ($self->{ResultNo}, $self->{GenerateNo}, $e_no, $self->{ENo}, $self->{LastResultNo}, $self->{LastGenerateNo}, $i_no, $source_name, $name, $kind_id, $strength) ));
    }
}
